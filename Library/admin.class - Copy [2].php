...
class admin
{
	...
	function create($firstname, $lastname, $username, $password) {
		...
		$query = mysql_query("INSERT INTO admin SET firstname ='".$sanitizer->sanitizeInput($firstname)."', lastname ='".$sanitizer->sanitizeInput($lastname)."', username ='".$sanitizer->sanitizeInput($username)."', password ='".$sanitizer->sanitizeInput($password)."'");
		if ($query) {
